...
let GameEngine={
	...
	keybord:function(){
		...
		window.onkeydown=e=>{
			...
			if(e.keyCode==37){
				...
				yspeed = 10
				//Plane().ele.style.top=Plane().ele.offsetTop+10+"px";
			}
		}
		window.onkeyup=e=>{
			e=e||event;
		...
		setInterval(()=>{
			let x=Plane().ele.offsetLeft+xspeed
			if(x<0) x=0;
			if(x >GameEngine.ele.offsetWidth-Plane().ele.offsetWidth){
				x = GameEngine.ele.offsetWidth-Plane().ele.offsetWidth-1;
			};
			Plane().ele.style.top=Plane().ele.offsetTop+yspeed+"px";
	...
	crash:function(){
		let timer=setInterval(()=>{
			for(let i=0;i<GameEngine.allEnemys.length;i++){  //遍历敌机
				...
				//判断我的飞机与敌机是否碰撞
				if(isCrash(GameEngine.allEnemys[i].ele,Plane().ele)){
					//console.log(1)
					clearInterval(timer);
					Plane().boom(function(){
						let name=prompt("请留下您的大名,您当前的分数是"+GameEngine.totalscore);
						ajax({
							type: "post",
